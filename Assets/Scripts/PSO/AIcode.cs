...
public class AIcode : MonoBehaviour {
	...
	void continueGame()
	{
			...
			Vector3 dest = transform.position + 2f * Random.value * (indBest - transform.position) + 2 * Random.value * (target.position - transform.position);
